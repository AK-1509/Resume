...
export function ExperienceStub({ experience }: { experience: Experience }) {
  return (
    <div className="flex items-baseline gap-3 border-l-2 border-rule/40 py-2 ps-5 opacity-70">
      <span className="metadata shrink-0 text-soft">
        {formatDuration(experience.startDate, experience.endDate)}
      </span>
      <span className="truncate font-sans text-ui text-soft">{experience.title}</span>
    </div>
  );
}
